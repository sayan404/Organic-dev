...
export async function GET(req: NextRequest) {
    try {
        ...
        const organsData = await EligibleDonorWithOrganDetail.find({
            organType,
...
export async function POST(req: NextRequest) {
    try {
        ...
        const newEligibleDonorWithOrganDetail =
            new EligibleDonorWithOrganDetail({
                donorType,
                nomineeName,
                nomineeContactNumber,
                donorId,
        ...
        try {
            if (formData) {
                const fileTextData = await uploadFiles(
                    formData,
                    newEligibleDonorWithOrganDetail._id
                );
